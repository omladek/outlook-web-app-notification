...
/**
 * Check if the Web app has loaded (the folder tree exists), if not check again later.
 */
...
function init() {
    ...
    UNREAD_EMAILS_TOTAL = getUnreadEmailsTotal();
    LAST_NOTIFICATION_ID = getNotificationId();

    debug('LAST_NOTIFICATION_TEXT: ' + LAST_NOTIFICATION_TEXT);
    debug('UNREAD_EMAILS_TOTAL: ' + UNREAD_EMAILS_TOTAL);
    debug('LAST_NOTIFICATION_ID: ' + LAST_NOTIFICATION_ID);
...
function getNotificationId(currentUnread) {
    let id = UNREAD_EMAILS_TOTAL + LAST_NOTIFICATION_TEXT;

    if (currentUnread) {
        id = currentUnread + LAST_NOTIFICATION_TEXT;
    }
